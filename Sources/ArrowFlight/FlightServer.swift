...
public protocol ArrowFlightServer: Sendable {
  ...
  func listFlights(_ criteria: FlightCriteria, writer: FlightInfoStreamWriter)
    async throws
  ...
  func doGet(_ ticket: FlightTicket, writer: RecordBatchStreamWriter)
    async throws
  func doPut(
    _ reader: RecordBatchStreamReader, writer: PutResultDataStreamWriter)
    async throws
  func doExchange(
    _ reader: RecordBatchStreamReader, writer: RecordBatchStreamWriter)
    async throws
}
  ...
  Arrow_Flight_Protocol_FlightServiceAsyncProvider
{
  ...
  func handshake(
    ...
    responseStream: GRPC.GRPCAsyncResponseStreamWriter<
      Arrow_Flight_Protocol_HandshakeResponse
    >,
    context: GRPC.GRPCAsyncServerCallContext
  ) async throws {
    throw ArrowFlightError.notImplemented()
  ...
  func listFlights(
    ...
    responseStream: GRPC.GRPCAsyncResponseStreamWriter<
      Arrow_Flight_Protocol_FlightInfo
    >,
    context: GRPC.GRPCAsyncServerCallContext
  ) async throws {
  ...
  func getFlightInfo(
    ...
    if let server = arrowFlightServer {
      return try await server.getFlightInfo(FlightDescriptor(request))
        .toProtocol()
    }
  ...
  func listActions(
    ...
    responseStream: GRPC.GRPCAsyncResponseStreamWriter<
      Arrow_Flight_Protocol_ActionType
    >,
    context: GRPC.GRPCAsyncServerCallContext
  ) async throws {
    if let server = arrowFlightServer {
  ...
  internal var interceptors:
    Arrow_Flight_Protocol_FlightServiceServerInterceptorFactoryProtocol?
  { nil }
