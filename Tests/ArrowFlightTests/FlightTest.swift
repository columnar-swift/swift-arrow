...
func makeSchema() -> ArrowSchema {
  ...
  return schemaBuilder.addField("col1", type: .float64, isNullable: true)
...
nonisolated(unsafe) var flights: [String: FlightInfo] = [:]
final class MyFlightServer: ArrowFlightServer {
  ...
  func doPut(
    ...
    for try await rbData in reader {
      ...
      flights[key] = try FlightInfo(
        schemaToMessage(rb.schema), endpoints: [], descriptor: rbData.1)
      XCTAssertEqual(rb.schema.fields.count, 3)
      XCTAssertEqual(rb.length, 4)
  ...
  func doGet(
    _ ticket: ArrowFlight.FlightTicket,
    writer: ArrowFlight.RecordBatchStreamWriter
  ) async throws {
  ...
  func listFlights(
    _ criteria: ArrowFlight.FlightCriteria,
    writer: ArrowFlight.FlightInfoStreamWriter
  ) async throws {
    XCTAssertEqual(
      String(bytes: criteria.expression, encoding: .utf8),
      "flight criteria expression")
  ...
  func listActions(_ writer: ArrowFlight.ActionTypeStreamWriter) async throws {
    try await writer.write(
      FlightActionType("clear", description: "Clear the stored flights."))
    try await writer.write(
      FlightActionType("shutdown", description: "Shut down this server."))
  }
  ...
  func doAction(_ action: FlightAction, writer: ResultStreamWriter) async throws
  {
    ...
    XCTAssertEqual(
      String(bytes: action.body, encoding: .utf8)!, "healthcheck body")
    try await writer.write(
      FlightResult("test_action result".data(using: .utf8)!))
  }
}
...
struct FlightServerImpl {
  ...
  nonisolated(unsafe) static var group: MultiThreadedEventLoopGroup?
...
public class FlightClientTester {
  ...
  func listFlightsTest() async throws {
    let flightCriteria = FlightCriteria(
      "flight criteria expression".data(using: .utf8)!)
    var numCalls = 0
    try await client?.listFlights(
      flightCriteria,
      closure: { data in
  ...
  func doActionTest(_ type: String, actionBody: Data) async throws {
    ...
    XCTAssertEqual(
      String(bytes: actionResults[0].body, encoding: .utf8),
      "test_action result")
  }
      ...
      readerResultClosure: { rb in
        ...
        case .success(let table):
          for column in table.columns {
            switch column.type {
            case .float64:
              let doubleArray = try column.data() as? ChunkedArray<Double>
    ...
    try await client?.doGet(
      ...
      flightDataClosure: { flightData in
        switch reader.fromMessage(
          flightData.dataHeader, dataBody: flightData.dataBody,
          result: arrowResult)
        {
        case .success:
          numCall += 1
  ...
  func doPutTest(_ cmd: String) async throws {
    ...
    let rb = try makeRecordBatch()
    var numCall = 0
    try await client?.doPut(
      descriptor, recordBatches: [rb],
      closure: { _ in
        numCall += 1
      })
  ...
  func testFlightServer() async throws {
    ...
    let secondTask = Task {
      ...
      try await clientImpl.doActionTest(
        "healthcheck", actionBody: Data("healthcheck body".utf8))
